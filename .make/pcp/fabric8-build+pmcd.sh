...
# We can log in plaintext to stdout also, even though ENV uses
...
sleep 5 # give time for pmcd's startup messages, so it doesn't intermix with ENV's


exec bin/fabric8-build ${1+"$@"}
